...
| File | Mô tả |
|------|-------|
| `01-fortio-deploy.yaml` | Deployment cho Fortio client: image `fortio/fortio:1.74.0` (pinned), resource limits, nodeSelector `role: benchmark` |
...
```bash
# Lấy tên pod Fortio
FORTIO_POD=$(kubectl get pod -n benchmark -l app=fortio -o jsonpath='{.items[0].metadata.name}')

# Chạy load test
kubectl exec -n benchmark $FORTIO_POD -- \
...
| `-t` | Thời gian chạy test | 180s (measurement), 30s (warmup) |
...
- **nodeSelector:** `role: benchmark` — đảm bảo node group có label này (plan §4.5).
