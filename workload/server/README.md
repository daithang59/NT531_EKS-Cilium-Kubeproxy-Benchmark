...
| File | Mô tả |
|------|-------|
| `01-namespace.yaml` | Tạo namespace `benchmark` — không gian riêng cho toàn bộ workload benchmark |
...
- **Service:** ClusterIP (không expose ra ngoài) — Fortio client gọi qua địa chỉ `echo-svc.benchmark.svc.cluster.local`.

## Lưu ý

- File được đánh số thứ tự (`01-`, `02-`, `03-`) để `kubectl apply -f` xử lý đúng thứ tự.
- Namespace `benchmark` phải tồn tại trước khi deploy echo server.
